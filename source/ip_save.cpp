...
static void blockVanillaLoad(spm::nandmgr::SaveFile * save)
{
    ...
    u32 nameSize = wii::string::strlen(name) + 1; // include null byte

    // Move back name so the mod can read it
    wii::string::memmove(name + newMapnameOffset, name, nameSize);
...
static void undoVanillaBlock(spm::nandmgr::SaveFile * save)
{
    ...
    u32 nameSize = wii::string::strlen(trueName) + 1; // include null byte
    ...
    wii::string::memmove(name, trueName, nameSize);
}
...
{
    wii::stdio::sprintf(gpSaveName, "[IP] %s", chosenSaveName);
}
